...
function resolveSecurePath(filename) {
    const resolvedPath = path.resolve(PROJECT_DIR, filename);
    if (!resolvedPath.startsWith(path.resolve(PROJECT_DIR))) {
        throw new Error("Access denied: Cannot access files outside of PROJECT_DIR");
    }
    return resolvedPath;
}

export async function executePowerShell(command, bypassSafety = false) {
